...
lines.append("")
lines.append("**Why lambda=0.05 produces lower F1 than lambda=0.15 (counterintuitive result):**")
lines.append("")
lines.append("A lower lambda means the trust score must drop further before a vehicle is")
lines.append("flagged. Because the trust update is cumulative, a Sybil node that sends even")
lines.append("a few beacons with speed inside the normal range will temporarily push its")
lines.append("trust score above 0.05, escaping detection. At lambda=0.15 the trust score")
lines.append("of a consistently anomalous node (dropping by 10% each step) reaches the")
lines.append("threshold in ceil(ln(0.3)/ln(0.9)) = 12 steps, which is fast enough to catch")
lines.append("all Sybil nodes before the simulation ends. At lambda=0.05 that threshold")
lines.append("requires ceil(ln(0.1)/ln(0.9)) = 22 steps, but if the Sybil node sends even")
lines.append("two legitimate-speed beacons in a row, the score recovers enough to stay")
lines.append("above 0.05, and the node escapes the flag. This explains why F1 drops at")
lines.append("very low lambda values rather than rising.")
lines.append("")
