...
h()
rf_f1  = round(float(means["Random Forest"]),  3)
rf_t   = round(float(times["Random Forest"]),  2)
tas_f1 = round(float(means["TASER Bayesian Trust"]), 3)
tas_t  = round(float(times["TASER Bayesian Trust"]), 2)
h(f"With out-of-sample (OOS) evaluation, TASER (F1={tas_f1}, {tas_t}s) strictly")
h(f"dominates RF (F1={rf_f1}, {rf_t}s) on both quality and speed. Only TASER and IQR")
...
h()
lstm_f1_20 = round(float(df[(df["detector"]=="LSTM") & (df["sybil_rate"]==20)]["f1"].values[0]), 3)
lstm_f1_30 = round(float(df[(df["detector"]=="LSTM") & (df["sybil_rate"]==30)]["f1"].values[0]), 3)
h("The transition from F1 = 0 to F1 > 0 occurs between 10% and 20% Sybil rate.")
h(f"At 20% (21.0% of records), LSTM achieves F1 = {lstm_f1_20} (mean over 5 seeds).")
h(f"At 30% (28.1%) it reaches F1 = {lstm_f1_30}. This threshold behavior is consistent")
h("with the class imbalance literature, where models typically require a minority")
h("fraction above 10-15% to train reliably without oversampling techniques like SMOTE.")
...
rf_std_5 = round(float(df[(df["detector"]=="Random Forest") & (df["sybil_rate"]==5)]["f1_std"].values[0]), 4)
h(f"At 5% sybil rate, RF 5-fold CV produces f1_std = {rf_std_5} (mean across 5 seeds),")
h("the highest variance of any scenario. This is expected and not a defect in the method.")
h()
